...
$.formValidator = 
{
	...
	setTipState : function(elem,showclass,showmsg)
	{
		...
		if(showmsg==null || showmsg=="")
		{
			//tip.hide();
			tip.removeClass();
			if(showclass=="onShow")tip.addClass( "onEmptyShow" );
			else tip.addClass( showclass );
			tip.html( "&nbsp;" );
		}
		else
		{
	...
	{
		if(!returnObj||!returnObj.id||!$("#"+returnObj.id).get(0))return false;
	    var id = returnObj.id;
		var elem = $("#"+id).get(0);
	...
	showAjaxMessage : function(returnObj)
	{
		...
		else
		{
			if(setting.isvalid!=undefined && !setting.isvalid){
				//$.formValidator.setTipState(elem,"onCorrect",elem.settings[0].oncorrect);
				...
				//elem.lastshowmsg   = elem.settings[0].oncorrect;
				//setting.isvalid = true;
			}
    ...
    getLength : function(id)
    {
        ...
        switch(sType)
		{
			...
			case "checkbox":
			case "radio": 
				var key = srcjo.attr("key");
				if(key&&key!=""){
					len = $("input[type='"+sType+"'][key='"+srcjo.attr("key")+"']:checked").length;
				}else{
	...
	pageIsValid : function (validatorgroup)
	{
		...
		var theerrarr = new Array();
		var returnObj,setting;
		var error_tip = "^"; 	

		var initConfig = $.formValidator.getInitConfig(validatorgroup);
		if(!initConfig.validobjectids||$.trim(initConfig.validobjectids)==""){
			return false;
		}
		var jqObjs = $(initConfig.validobjectids);
		jqObjs.each(function(i,elem)
		{
			if(!elem||!elem.settings[0]||!elem.settings[0].bind){
				return true;
				//return false;
			}
				...
				if(returnObj)
				{
					...
					if (!returnObj.isvalid) {
						...
						if(thefirstid == ""){
							...
							theerrarr[theerrarr.length] = {"id":returnObj.id,"errmsg":(returnObj.errormsg==""?returnObj.setting.onerror:returnObj.errormsg)};
						}
					}
		...
		if(isvalid)
		{
            ...
            $.formValidator.showMessage(isvalid);
		...
		else
		{
			...
			$.each(theerrarr,function(i,item){
				var obj = $("#"+item["id"]).get(0);
				initConfig.onerror(item["errmsg"],obj);
			});
		}
	...
	ajaxValid : function(returnObj)
	{
		...
		if(setting.otherFlag){
			setting.data = setting.otherData();
		}
		$.ajax(
		{	
			mode : "abort",
		...
		switch(sType)
		{
			...
			case "radio":
				...
				else if (type =="date" || type =="datetime")
				{
					...
					if(setting.canEmpty&&val==""){lb_go_on = true};
	...
	return this.each(function(e)
	{
		...
		if(srcTag == "input" || srcTag=="textarea")
		{
			...
			jqobj.bind(setting.triggerevent, function(){
				...
				if(!returnObj||returnObj==null){return false;}
				if(returnObj.ajax >= 0) 
				{
		...
		else if (srcTag == "select")
		{
			...
			//失去焦点
			if(setting&&setting.execChange){
				jqobj.bind("blur",function(){jqobj.trigger("change")});
			}else{
				//jqobj.bind("blur",function(){jqobj.trigger("blur")});
			}
			//选择项目后触发
