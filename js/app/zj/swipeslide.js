var SwipeSlide = function(container, options){
  ...
  this.container   = Zepto(container).addClass('ui-swipeslide').addClass('ui-swipeslide-'+(this.isVertical ? 'vertical' : 'horizontal'))
...
SwipeSlide.prototype = {
  ...
  page: function(index) {
    ...
    if (this.currentPage != newPage) {
      if(Zepto.isFunction(this.options.beforeChange)) this.options.beforeChange(this, this.currentPage, newPage)
      this.currentPage = newPage
      callback = Zepto.proxy(this.callback, this)
    } else if (this.options.autoPlay){
      callback = Zepto.proxy(this.autoPlay, this)
    }
  ...
  autoPlay:  function(){
    ...
    this.timeout = setTimeout(Zepto.proxy(fn, this), this.options.autoPlay * 1000) 
  },
  ...
  addEventListeners: function(){
    ...
    this.reel
      .on(this.events.start, Zepto.proxy(this.touchStart, this))
      .on(this.events.move,  Zepto.proxy(this.touchMove, this))
      .on(this.events.end,   Zepto.proxy(this.touchEnd, this))
      
    // bind listeners to any elments with '.prev', '.next', '.first' or '.last' class 
    this.container
      .on(this.events.click, '.next',  Zepto.proxy(this.next,  this))
      .on(this.events.click, '.prev',  Zepto.proxy(this.prev,  this))
      .on(this.events.click, '.first', Zepto.proxy(this.first, this))
      .on(this.events.click, '.last',  Zepto.proxy(this.last,  this))

    // recalculate dimension on window resize or orientation change
    Zepto(window).on('resize', Zepto.proxy(this.setup, this))
  },
  ...
  callback: function(){
    ...
    if (Zepto.isFunction(this.options.afterChange)) this.options.afterChange(this, this.currentPage)
    if (this.options.autoPlay) this.autoPlay()
  },
  ...
  setupBulletNavigation: function() {
    this.navBullets = Zepto('<ul class="ui-swipeslide-bullets"></ul>')
    for (i=0; i<this.numPages; i++) this.navBullets.append('<li data-index="'+i+'">'+(i+1)+'</li>')
    if (this.options.bulletNavigation == 'link') {
      this.navBullets.on(this.events.click, 'li', Zepto.proxy(function(e){
        this.page(parseInt(Zepto(e.currentTarget).data('index'), 10))
      }, this))
    }
...
Zepto.extend(SwipeSlide3D.prototype, {
  setup: function() {
    var fn = this.isVertical ? 'height' : 'width'
    this.dimension = this.container[fn]()
    this.tolerance = this.options.tolerance * this.dimension / 2
    ...
    this.slides.each(Zepto.proxy(this.positionSlide, this))
    this.move(0,0)
  },
  ...
  positionSlide: function(i, slide){
    Zepto(slide).animate({ rotate3d: this.vectorsWithDeg(i*this.alpha), translate3d: '0,0,'+this.radius+'px' }, {duration: 0})
  },
...
// zepto plugin
;(function(Zepto) {
  Zepto.fn.swipeSlide = function(options) {
