function submitScore(fmt,hash,score){
	//alert("submit");
	var form=document.getElementById("myformid");
	form["time"].value=fmt;
	form["checkcode"].value=hash;
...
(function() {
			...
			loadImages: function(b, c) {
				...
				if (a.image.loadedImg) {
					...
					for (k = 0; k < a.image.loadFrame.length; k++) {
						jsGame.canvas.drawImage(a.image.loadImgId, 47 + a.image.loadFrame[k].frames[a.image.loadFrame[k].step++] * 7, 3, 7, 7, d + a.image.loadFrame[k].x, f + a.image.loadFrame[k].y, 7, 7);
	...
	jsGame = {
				...
				a.ajax.xhrObj.xhr.onreadystatechange = function() {
					...
					if (a.ajax.xhrObj && a.ajax.xhrObj.xhr.readyState == 4) {
						if (a.ajax.xhrObj.xhr.status == 200) {
							...
							default:
								c = a.ajax.xhrObj.xhr.responseText;
								break;
							case "JSON":
		...
		objExtend: function() {
			...
			for (; c < d; c++) if ((k = arguments[c]) != null) for (var r in k) {
				var s = b[r],
					p = k[r];
				if (b !== p) if (f && p && typeof p === "object" && !p.nodeType) b[r] = this.objExtend(f, s || (p.length != null ? [] : {}), p);
		...
		clone: function(b) {
			var c;
			b = b || [];
			if (typeof b == "object") if (b.length) {
				c = [];
				for (var d = 0; d < b.length; d++) if (b[d].length && b[d].length > 0) {
					c[d] = [];
		...
		classes: {
			init: function(b) {
				...
				b.classes.timer.prototype.start = function(c) {
					if (c) {
		...
		commandFuns: function() {
			...
			return {
				...
				rangeRegisterNotify: function(c, d) {
					for (var f = 0; f < d.length; f++) jsGame.commandFuns.registerNotify(c, d[f])
				},
				...
				inArray: function(c, d) {
					var f, k = d.length;
					for (f = 0; f < k; f++) if (c == d[f]) return f;
					return -1
				},
		...
		canvas: function() {
			...
			return {
				...
				drawCache: function(e, i, j, h, l, q, m, n, t, u) {
					if (e = a.canvas.cavansDoms[e]) if (h) if (l) if (u) {
						d = o.getAnchor(q, m, n, t, u);
				...
				drawRegion: function(e, i, j, h, l, q, m, n) {
					switch (q) {
					default:
						c.setTransform(1, 0, 0, 1, m, n);
						break;
					case g.canvas.trans.TRANS_ROT90:
						c.setTransform(0, 1, -1, 0, l + m, n);
						break;
					...
					}(jsGame.getImage(e) ? b.drawImage : b.drawCache)(e, i, j, h, l, 0, 0, h, l);
				...
				drawNumber: function(e, i, j, h, l, q, m, n, t) {
					...
					n = n ? n : j;
					t = t ? t : h;
					if (m) for (m = 0; m < u; m++) b.drawImage(i, parseInt(e.charAt(m)) * j, 0, j, h, l + m * n, q, n, t);
					else for (m = u - 1; m >= 0; m--) b.drawImage(i, parseInt(e.charAt(m)) * j, 0, j, h, l - (u - 1 - m) * n, q, n, t, jsGame.graphics.ANCHOR_RT);
					return b
				},
				moveTo: function(e, i) {
					c.moveTo(e, i);
		...
		audio: function() {
			...
			return {
				...
				fuckAudio: function(c) {
					if (a.audio.audios[c] && a.audio.audios[c].paused) {
						a.audio.fuckSkip++;
		...
		initAudio: function(b) {
			if (b.length > 0) {
				a.audio.audios = [];
				for (var c, d, f, k, r = 0; r < b.length; r++) if ((c = b[r]) && c.id != "" && c.src != "") {
					d = c.loop;
					f = c.preload;
					k = c.autoplay;
