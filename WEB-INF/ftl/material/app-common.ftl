...
<!--
	<#if pageFinder ?? && pageFinder.rowCount ??>
		<div class="">总记录数：<font color ="red">${pageFinder.rowCount}</font></div>
	</#if>
	-->
	<div id="turn_page">
		...
		<#if (pageFinder.pageCount > 1) >
			...
			<#else>
		    <font class="page_noclick"><a href="#" onClick="queryPage(${pageFinder.pageNo-1})"  title="上一页"><span>上一页</span></a></font>
			...
			<#elseif ((pageFinder.pageCount - pageFinder.pageNo) < 5) >
				<#list 1..9 as row>
					<#if ((pageFinder.pageCount - pageFinder.pageNo) == (9- row)) >
						<!-- 选中时的样式 -->
						<font class="page_item_current">${pageFinder.pageCount  - 9 + row}</font>
					<#else>
						<a href="#" onClick="queryPage(${pageFinder.pageCount - 9 + row})" class="page_item"> ${pageFinder.pageCount  - 9 + row}</a>
						<!-- 默认的样式 -->
					</#if>
				</#list>
			...
			<#else>
				<#list 1..10 as row>
					...
					<a>	...</a>
					<#elseif (row == 9) >
					<font class="page_item_current"><a href="#" onClick="queryPage(${pageFinder.pageCount -1 })"  > ${pageFinder.pageCount -1}</a></font>
					<#elseif (row == 10) >
					<font class="page_item_current"><a href="#" onClick="queryPage(${pageFinder.pageCount})" > ${pageFinder.pageCount}</a></font>
					<#else>
						...
						<#else>
							...
							<#if (row == 4) >
								<!-- 选中时的样式 -->
								<font class="page_item_current">${pageFinder.pageNo-4+row}</font>
							<#else>
								<!-- 默认的样式 -->
								<a href="#" onClick="queryPage(${pageFinder.pageNo-4+row})" class="page_item"> ${pageFinder.pageNo-4+row}</a>
							</#if>
			...
			<#else>
			    <a href="#" onClick="queryPage(${pageFinder.pageCount})"   title="下一页"  class="page_item">尾页</a>
				<a href="#" onClick="queryPage(${pageFinder.pageNo+1})"   title="下一页"  class="page_item">下一页</a>
			</#if>
			
			<!--
			<span class="jum-page">
				到第
				<input style="width:40px" id="query.page" size="5" name="query.page" type="text" maxlength="5" value="${pageFinder.pageNo}"  onkeyup="value=value.replace(/[^\d]/g,'')"/>
				页
			</span>
			<a class="submit-page ft-sz-12" href="#"  onClick="queryPage(0);">跳转</a>
			-->
				<span>每页显示行数:<input  style="height:20px;"  type="text" name="pageSize" id="pageSizeInput"  size="5" value="${pageFinder.pageSize?default("")}" onkeydown="changePageSizeEnter(event)" />&nbsp;<input type="button"  onclick="changePageSize()" value="Go"></span>
		<#else>
			<div>每页显示行数:<input style="height:20px;"   type="text" name="pageSize" id="pageSizeInput"  size="5" value="${pageFinder.pageSize?default("")}" onkeydown="changePageSizeEnter(event)" />&nbsp;<input type="button"  onclick="changePageSize()" value="Go"></div>
		</#if>
