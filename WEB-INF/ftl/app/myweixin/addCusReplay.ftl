...
<div id="reply_keyword" class="r_con_wrap">
        ...
        <form id="keyword_reply_form" class="r_con_form" method="post" action="${BasePath}/addCustomReplay.kq?type=${type}">
            <div class="rows">
                <label>关键词</label>
                <span class="input"><textarea name="Keywords" class="keywords" notnull></textarea><div class="tips">多个关键词，请每行填写一个</div></span>
                <div class="clear"></div>
            </div>
            <div class="rows">
                <label>匹配模式</label>
                <span class="input">
                    <input type="radio" name="PatternMethod" value="0" checked />精确匹配<span class="tips">（用户输入的文字和此关键词一样才会触发）</span><br />
                    <input type="radio" name="PatternMethod" value="1"  />模糊匹配<span class="tips">（只要用户输入的文字包含此关键词就触）</span><br />
                </span>
                <div class="clear"></div>
            </div>
            <div class="rows">
                ...
                <span class="input"><select name="ReplyMsgType">
                    ...
                    <option value="2" selected>图文消息</option>
                </select></span>
                <div class="clear"></div>
            </div>
		...
		<#if item.items?size==1>【单图文】<#else>【多图文】</#if>
            ...
            <div class="rows">
                <label></label>
                <span class="input"><input type="submit" class="btn_green" name="submit_button" value="提交保存" /><a href="javascript:window.history.go(-1);" class="btn_gray">返回</a></span>
                <div class="clear"></div>
            </div>
