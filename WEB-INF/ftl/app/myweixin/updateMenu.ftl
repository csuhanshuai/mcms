...
<div class="r_nav">
    <ul>
        ...
        <li><a href="${BasePath}/myweixin.kq?type=binding">公众账号绑定</a></li>
...
<div class="m_menu">
    ...
    <div class="form">
            ...
            <#if fmenus??>
                ...
                <ul>
                    ...
                    <#list amenus as sub>
                        <#if sub.pid==item.id>
                        <li MId="${sub.id}">
                        <div class="title"><img src="http://static.ptweixin.com/member/images/ico/jt.gif" /> ${sub.menuname}</div>
                        <div class="opt">
        ...
        <div class="m_righter">
        <form id="wechat_menu_form" class="mod_form"  action="${BasePath}/updateMenu.kq" method="post">
            <h1>修改菜单</h1>
            <div class="opt_item">
                <label>菜单名称：</label>
                <span class="input"><input type="text" name="menuname" value="${cmenu.menuname?default('')}" class="form_input" size="15" maxlength="15" notnull /> <font class="fc_red">*</font></span>
                <div class="clear"></div>
            </div>
                                <div class="opt_item">
                <label>消息类型：</label>
                <span class="input"><select name="replayType">
                    <option value="1" <#if cmenu.replyType=="1">selected</#if>>文字消息</option>
                    <option value="2" <#if cmenu.replyType=="2">selected</#if>>图文消息</option>
                    <option value="3" <#if cmenu.replyType=="3">selected</#if>>连接网址</option>
                </select></span>
                <div class="clear"></div>
            </div>
            <div class="opt_item" id="text_msg_row">
                <label>文字内容：</label>
                <span class="input"><textarea name="TextContents" value="">${cmenu.content?default('')}</textarea></span>
                <div class="clear"></div>
            </div>
            <div class="opt_item" id="img_msg_row">
                ...
                <#if ptList??>
                <#list ptList as item>
                <option value='${item.id}' <#if cmenu??&&cmenu.msgid==item.id>selected</#if>><#if item.items?size==1>【单图文】<#else>【多图文】</#if>${item.items[0].title?default('')}</option>
                </#list>
		</#if>
            ...
            <div class="opt_item" id="url_msg_row">
                <label>链接网址：</label>
                <span class="input"><input type="text" name="Url"  value="${cmenu.url?default('')}" class="form_input" size="35" maxlength="200" /></span>
                <div class="clear"></div>
            </div>
            <div class="opt_item">
                <label></label>
                <span class="input"><input type="submit" class="btn_green btn_w_120" name="submit_button" value="修改菜单" /></span>
                <div class="clear"></div>
            </div>
            <input type="hidden" name="id" value="${cmenu.id}">
